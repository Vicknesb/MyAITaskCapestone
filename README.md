...
![Statements](https://img.shields.io/badge/statements-99.66%25-brightgreen)
![Branches](https://img.shields.io/badge/branches-97.77%25-brightgreen)
![Functions](https://img.shields.io/badge/functions-91.66%25-brightgreen)
![Lines](https://img.shields.io/badge/lines-99.66%25-brightgreen)
![Security](https://img.shields.io/badge/security-11%2F11%20fixed-brightgreen)

> Coverage generated by `npm run test:coverage` — tracked in [`coverage-summary.json`](coverage-summary.json). All metrics exceed configured thresholds (statements/functions/lines ≥ 80%, branches ≥ 75%).  
> Security: all 11 audit findings resolved — see [`docs/SECURITY-AUDIT.md`](docs/SECURITY-AUDIT.md).
...
Coverage is measured with Vitest + v8. Run it yourself:

```bash
npm run test:coverage
```

Latest measured results (also in [`coverage-summary.json`](coverage-summary.json)):
...
157 tests across 15 test files — all passing. Test suite covers:

- Auth (register, login, JWT sessions, session renewal, logout, rate limiting)
- Repository connect / disconnect / cascade delete / file browser / path traversal
- Metrics per-repo and dashboard aggregation (date range, type filter, null fallbacks)
- Sync engine (happy path, 409 empty repo, all GitHub error codes, error sanitization)
- Middleware (auth guard, Bearer prefix, token renewal, sliding-window sessions)
- Crypto (AES-256-GCM round-trip, HKDF key derivation, startup guards, key length validation)
- Input validation (password/name length, date range cap, schema edge cases)
- Environment validation (`validateEnv` with missing/partial/full env sets)
...
| `npm run test:coverage` | Run tests with v8 coverage (thresholds: 80% stmts/funcs/lines, 75% branches) |
