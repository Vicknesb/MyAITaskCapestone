...
![Statements](https://img.shields.io/badge/statements-99.65%25-brightgreen)
![Branches](https://img.shields.io/badge/branches-97.65%25-brightgreen)
![Functions](https://img.shields.io/badge/functions-90.9%25-brightgreen)
![Lines](https://img.shields.io/badge/lines-99.65%25-brightgreen)

> Coverage figures above are generated by `npm run test:coverage` and tracked in [`coverage-summary.json`](coverage-summary.json). All four metrics exceed the configured thresholds (statements/functions/lines ≥ 80%, branches ≥ 75%).
...
149 tests across 14 test files — all passing. Test suite covers:

- Auth (register, login, JWT sessions, session renewal, logout)
- Repository connect / disconnect / cascade delete / file browser / path traversal
- Metrics per-repo and dashboard aggregation (date range, type filter)
- Sync engine (happy path, 409 empty repo, error handling)
- Middleware (auth guard, Bearer prefix, token renewal)
- Crypto utilities (AES-256-GCM round-trip, startup guards, key length validation)
- Input validation boundaries (password length, name length, date range cap)
